...
function onOpen() {
  const env = String(PropertiesService.getScriptProperties().getProperty('ASAAS_ENV') || 'SANDBOX').trim().toUpperCase();
  const isProduction = env === 'PRODUCTION';
  const ui = SpreadsheetApp.getUi();
  const menu = ui.createMenu('🛍️ Loja do Grêmio')
    ...
    .addItem(isProduction ? '💳 Consultar parcelamento Asaas' : '💳 Testar parcelamento da linha selecionada', 'testarParcelamentoCartaoSelecionado')
    .addItem('🔄 Sincronizar pagamentos agora', 'sincronizarPagamentosAsaas');

  // Ferramentas exclusivas de homologação: desaparecem automaticamente em produção.
  if (!isProduction) {
    menu
      .addItem('🔑 Verificar/criar chave Pix Sandbox', 'verificarOuCriarChavePixSandbox')
      .addItem('🧪 Simular pagamento Sandbox (linha selecionada)', 'simularPagamentoSandboxSelecionado');
  }
  ...
  menu
    ...
    .addSeparator()
    .addItem('🌐 Configurar ponte Vercel (passo a passo)', 'configurarPonteVercel')
...
function gerarRelacaoGrafica() {
  ...
  sh.getRange('A2:I2').merge().setValue(
    env === 'SANDBOX'
      ? 'AMBIENTE DE TESTES (SANDBOX) — NÃO ENVIAR ESTA RELAÇÃO À GRÁFICA'
      : 'RELAÇÃO OFICIAL PARA PRODUÇÃO'
  ).setBackground(env === 'SANDBOX' ? '#FFF1C7' : '#E8F5E9')
    .setFontColor(env === 'SANDBOX' ? '#7A4C00' : '#1B5E20')
  ...
  ss.toast(
    env === 'SANDBOX'
      ? `Relação de TESTE gerada com ${total} peça(s). Não envie à gráfica enquanto estiver em Sandbox.`
      : `Relação gerada com ${total} peça(s), pronta para conferência e envio à gráfica.`,
...
function marcarRelacaoGraficaComoEnviada() {
  ...
  if (env === 'SANDBOX') {
    SpreadsheetApp.getUi().alert(
      'Ambiente Sandbox',
      'A relação atual é de testes. Para evitar alterar o fluxo de produção por engano, esta função fica bloqueada enquanto ASAAS_ENV = SANDBOX.',
...
function configurarPonteVercel() {
  ...
  props.setProperty('VERCEL_PROXY_SECRET', secret);
  // Apenas um auxiliar para permitir conferência/rotação futura. O webhook da
  // Vercel continua validando o valor configurado na própria Vercel.
  props.setProperty('VERCEL_ASAAS_WEBHOOK_TOKEN', webhookToken);
  ...
  <style>
    body{font-family:Arial,sans-serif;margin:0;padding:22px;color:#143b60;background:#f7f9fb}
    h2{margin:0 0 8px;color:#063667}.lead{margin:0 0 18px;line-height:1.45}
    .warn{background:#fff4e8;border-left:4px solid #f57c00;padding:12px 14px;border-radius:10px;margin:0 0 18px;color:#7a4300}
    .card{background:#fff;border:1px solid #d9e2e8;border-radius:16px;padding:16px;margin:12px 0;box-shadow:0 5px 16px rgba(6,54,103,.06)}
    .n{font-weight:700;color:#063667;margin-bottom:5px}.hint{font-size:12px;color:#64748b;margin:4px 0 8px}
    code.name{display:block;background:#edf4fa;color:#063667;border-radius:9px;padding:10px;font-weight:700;user-select:all}
    textarea{width:100%;box-sizing:border-box;min-height:58px;resize:vertical;border:1px solid #ccd8e0;border-radius:9px;padding:10px;font:13px Consolas,monospace;color:#0f2740;background:#fbfdff}
    button{border:0;border-radius:10px;padding:9px 13px;font-weight:700;cursor:pointer;background:#0b67a3;color:#fff;margin-top:8px}.secondary{background:#e9f2f8;color:#063667}
    .footer{font-size:12px;color:#64748b;line-height:1.45;margin-top:16px}.ok{color:#2e7d32;font-weight:700}
  </style></head><body>
    <h2>🌐 Ponte Vercel — configuração organizada</h2>
    <p class="lead">Na Vercel você vai criar <strong>3 variáveis separadas</strong>. Em cada variável, o campo <strong>Name</strong> recebe somente o nome e o campo <strong>Value</strong> recebe somente o valor.</p>
    <div class="warn"><strong>Importante:</strong> não cole <code>NOME=VALOR</code> no campo Name. Não use a chave da API do Asaas em nenhuma destas três variáveis.</div>

    <div class="card">
      <div class="n">1. Name</div><code class="name">APPS_SCRIPT_API_URL</code>
      <div class="n" style="margin-top:12px">Value</div>
      <textarea id="v1" readonly>${esc(backendUrl)}</textarea>
      <button onclick="copyVal('v1')">Copiar somente o valor</button>
      <div class="hint">Exemplo de formato: https://script.google.com/macros/s/SEU_DEPLOYMENT_ID/exec</div>
    </div>

    <div class="card">
      <div class="n">2. Name</div><code class="name">APPS_SCRIPT_PROXY_SECRET</code>
      <div class="n" style="margin-top:12px">Value</div>
      <textarea id="v2" readonly>${esc(secret)}</textarea>
      <button onclick="copyVal('v2')">Copiar somente o valor</button>
      <div class="hint">Este segredo liga a Vercel ao Apps Script. Não compartilhe e não coloque no GitHub.</div>
    </div>

    <div class="card">
      <div class="n">3. Name</div><code class="name">ASAAS_WEBHOOK_TOKEN</code>
      <div class="n" style="margin-top:12px">Value</div>
      <textarea id="v3" readonly>${esc(webhookToken)}</textarea>
      <button onclick="copyVal('v3')">Copiar somente o valor</button>
      <div class="hint">Este NÃO é o ASAAS_API_KEY. Depois, no webhook do Asaas, use exatamente este mesmo valor como token de acesso.</div>
    </div>

    <div class="footer"><span class="ok">Depois de cadastrar as três variáveis:</span> faça um novo deploy na Vercel e abra <code>/api/health</code>. Os três campos de configuração devem aparecer como <code>true</code>.</div>
    <script>
      function copyVal(id){
  ...
  </body></html>`).setWidth(720).setHeight(760);

  SpreadsheetApp.getUi().showModalDialog(html, '🌐 Configurar ponte Vercel');
...
function mostrarDadosPonteVercel() {
  const props = PropertiesService.getScriptProperties();
  const backendUrl = ScriptApp.getService().getUrl() || '';
  const proxyConfigured = Boolean(props.getProperty('VERCEL_PROXY_SECRET'));
  const webhookConfigured = Boolean(props.getProperty('VERCEL_ASAAS_WEBHOOK_TOKEN'));

  const msg =
    'PONTE VERCEL — CONFERÊNCIA\n\n' +
    '1) APPS_SCRIPT_API_URL\n' + backendUrl + '\n\n' +
    '2) APPS_SCRIPT_PROXY_SECRET\n' + (proxyConfigured ? 'CONFIGURADO ✅' : 'NÃO CONFIGURADO ❌') + '\n\n' +
    '3) ASAAS_WEBHOOK_TOKEN\n' + (webhookConfigured ? 'GERADO ✅' : 'AINDA NÃO GERADO ❌') + '\n\n' +
    'Os segredos não são exibidos nesta tela. Para gerar uma configuração nova e copiar os valores em caixas separadas, use “Configurar ponte Vercel”.';
  ...
  return {
    ok: true,
    backendUrl: backendUrl,
    proxySecretConfigured: proxyConfigured,
    webhookTokenConfigured: webhookConfigured
  };
}
...
function getPublicData_() {
  ...
  const result = {
    ...
    mockups: {
      estudanteId: String(MOCKUP_IDS.estudante || ''),
      servidorPoloId: String(MOCKUP_IDS.servidorPolo || ''),
      servidorCarecaId: String(MOCKUP_IDS.servidorCareca || '')
    },
    asaasEnv: asaas.env,
 ...
 * Os IDs ficam no bloco MOCKUP_IDS, no início deste Code.gs.
 * Enquanto um ID estiver vazio, o site exibe um placeholder.
 */
function getMockupsPublicos() {
  return {
    estudante: getLogoDataUri_(MOCKUP_IDS.estudante),
    servidorPolo: getLogoDataUri_(MOCKUP_IDS.servidorPolo),
    servidorCareca: getLogoDataUri_(MOCKUP_IDS.servidorCareca)
  };
}
...
function getAsaasConfig_() {
  ...
  const envRaw = String(props.getProperty('ASAAS_ENV') || 'SANDBOX').trim().toUpperCase();
...
function asaasRequest_(path, method, body) {
  ...
  const options = {
    ...
    headers: {
      ...
      'User-Agent': 'GremioEstudantilPAA-Loja/7.2.7',
