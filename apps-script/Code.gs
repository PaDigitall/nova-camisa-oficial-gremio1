...
function gerarRelacaoGrafica() {
  ...
  const escreverProfissionais = (titulo,codigo) => {
    sh.getRange(row,1,1,6).merge().setValue(titulo).setBackground('#0D4F8B').setFontColor('#FFFFFF').setFontWeight('bold'); row++;
    const headers=['Tamanho','Nome do profissional','Nome na camisa','Cargo/Área/Função','Qtd.','Pedido'];
    sh.getRange(row,1,1,headers.length).setValues([headers]).setBackground('#D9E8F5').setFontWeight('bold').setHorizontalAlignment('center'); row++;
    const arr=profissionais.filter(i=>i.codigo===codigo).sort((a,b)=>rankTamanho(a.tamanho)-rankTamanho(b.tamanho)||a.tamanho.localeCompare(b.tamanho,'pt-BR')||nomePedido(a.pedido).localeCompare(nomePedido(b.pedido),'pt-BR')).map(i=>[
      i.tamanho||'—',nomePedido(i.pedido)||'—',i.nomeCamisa||'SEM PERSONALIZAÇÃO',i.cargoFuncao||'—',Number(i.quantidade||1),i.pedido
    ]);
    if(arr.length){ sh.getRange(row,1,arr.length,headers.length).setValues(arr); row+=arr.length; sh.getRange(row,1,1,4).merge().setValue('TOTAL').setFontWeight('bold').setHorizontalAlignment('right'); sh.getRange(row,5).setValue(arr.reduce((a,r)=>a+Number(r[4]||0),0)).setFontWeight('bold').setHorizontalAlignment('center'); row++; }
    else { sh.getRange(row,1,1,6).merge().setValue('Nenhum pedido pendente deste modelo.').setFontStyle('italic'); row++; }
    row++;
  };
  escreverProfissionais('PROFISSIONAIS — GOLA POLO','SERV-POLO');
  escreverProfissionais('PROFISSIONAIS — GOLA CARECA','SERV-CARECA');

  sh.getRange(row,1,1,9).merge().setValue('Relação gerada somente com pedidos PAGO/CONFIRMADO e ainda NÃO ENVIADOS à gráfica. Após enviar esta relação, use o menu “✅ Marcar relação atual como enviada à gráfica”.').setBackground('#F3F6F9').setFontColor('#4B5563').setFontStyle('italic').setWrap(true); row += 2;
