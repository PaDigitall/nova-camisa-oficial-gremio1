  ...
  for (let n = 1; n <= max; n++) {
    requests.push({
      ...
      headers: {
        ...
        'User-Agent': 'GremioEstudantilPAA-Loja/7.5.3',
...
function adminGetLots(token) {
  ...
  const lots = rows.filter(r => String(r[0] || '').trim()).map((r, idx) => {
    ...
    return {
      ...
      pedidosPagos: Number(r[5] || 0), receitaBruta: Number(r[6] || 0), taxas: Number(r[7] || 0),
      custoGrafica: Number(r[8] || 0), margem: Number(r[9] || 0),
      enviadoGrafica: String(r[10] || 'NÃO').trim().toUpperCase(), observacoes: String(r[11] || ''),
      atual: code === (publico === 'ESTUDANTE' ? currentEst : currentServ), temPedidos: lotHasOrders_(code)
    };
  });
